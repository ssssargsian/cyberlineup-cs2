    ...
    <header className="sticky top-0 z-40 mb-8 pt-1 backdrop-blur-xl">
      <div className="rounded-2xl border border-white/10 bg-[#0b0f18]/95 px-3 py-3 shadow-[0_18px_70px_rgba(0,0,0,0.42)] ring-1 ring-orange-400/5 sm:px-4">
        <div className="relative flex items-center justify-between gap-3">
          <Logo />
          <nav className="hidden shrink-0 gap-1 rounded-xl border border-white/8 bg-black/25 p-1 md:flex">
            {links.map((link) => {
          ...
          <button
            ...
            className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-black/30 text-slate-100 transition hover:border-orange-400/35 hover:bg-orange-500/10 md:hidden"
          >
            {isOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
          </button>

          {isOpen ? (
            <div className="absolute right-0 top-[calc(100%+0.75rem)] z-50 w-64 max-w-[calc(100vw-2rem)] rounded-2xl border border-white/10 bg-[#0b0f18] p-2 shadow-[0_24px_80px_rgba(0,0,0,0.5)] md:hidden">
              {links.map((link) => {
                const Icon = link.icon;
                const isActive = link.href === "/" ? pathname === "/" : pathname.startsWith(link.href);
